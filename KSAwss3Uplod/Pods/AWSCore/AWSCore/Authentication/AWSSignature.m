...
@property (nonatomic, assign) BOOL chunkedEncodingDisabled;
...
- (instancetype)initWithCredentialsProvider:(id<AWSCredentialsProvider>)credentialsProvider
                                   endpoint:(AWSEndpoint *)endpoint {
    return [self initWithCredentialsProvider:credentialsProvider
                                    endpoint:endpoint
                     chunkedEncodingDisabled:NO];
}
...
- (instancetype _Nonnull)initWithCredentialsProvider:(id<AWSCredentialsProvider> _Nonnull)credentialsProvider
                             ...
                             chunkedEncodingDisabled:(BOOL)chunkedEncodingDisabled {
    if (self = [super init]) {
        _credentialsProvider = credentialsProvider;
        _endpoint = endpoint;
        _chunkedEncodingDisabled = chunkedEncodingDisabled;
    }
...
- (NSString *)signS3RequestV4:(NSMutableURLRequest *)urlRequest
    ...
    if (nil != stream && (!self.chunkedEncodingDisabled)) {
    ...
    if (nil != stream) {
        BOOL chunkDisable = self.chunkedEncodingDisabled;
        AWSS3ChunkedEncodingInputStream *chunkedStream = [[AWSS3ChunkedEncodingInputStream alloc] initWithInputStream:stream
                                                                                              ...
                                                                                              chunkedEncodingDisabled:chunkDisable];
...
- (NSData *)getSignedChunk:(NSData *)data {
    if (self.chunkedEncodingDisabled) {
        return data;
    }
